...
class BrowserWebView extends WebView {
  const BrowserWebView({required Key? key, required String src, required double? width, required double? height, required OnLoaded? onLoaded, required this.options})
    : super(key: key, src: src, width: width, height: height, onLoaded: onLoaded);
  ...
  Widget builder(BuildContext context, ui.Size size, String contents) {
    ...
    return AbsorbPointer(
      child: RepaintBoundary(
        child: HtmlElementView(key: widget.key, viewType: 'iframe-$url'),
      ),
    );
  }
}
